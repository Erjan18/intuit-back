...
class AspiranturaView(APIView):
    def get(self,*args,**kwargs):
        name = Aspirantura.objects.all()
        serializer = AspiranturaSerializer(name,many=True)
...
class JobspirantView(APIView):
    def get(self,*args,**kwargs):
        name = Jobspirant.objects.all()
        serializer = JobspirantSerializer(name,many=True)
        return Response(serializer.data,status=status.HTTP_200_OK)
